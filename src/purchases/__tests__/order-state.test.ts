...
describe('purchase state projection', () => {
  ...
  it('prioritizes refund or financial hold over the original paid state', () => {
    expect(orderState({ ...pending, status: 'paid', access_id: 1, financially_blocked: true })).toBe('blocked')
    expect(orderState({ ...pending, status: 'refunded', access_id: 1, financially_blocked: true })).toBe('refunded')
    expect(orderState({ ...pending, status: 'cancelled' })).toBe('failed')
  })
